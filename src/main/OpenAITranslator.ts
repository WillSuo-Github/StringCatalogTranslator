...
class OpenAITranslator {
  ...
  private translationMemory: Map<string, string>; // key -> translation for consistency
  ...
  async translateStringFilePaths(
    filePaths: string[],
  ...
  private async translateStringFilePath(
    filePath: string,
    progressCallback: (progress: string) => void,
  ): Promise<void> {
    ...
    for (const key in fileContent.strings) {
      ...
      if (
        stringInfo.localizations[fileContent.sourceLanguage]?.stringUnit
          .state === 'new'
      ) {
        sourceValue =
          stringInfo.localizations[fileContent.sourceLanguage]?.stringUnit
    ...
    await this.processBatchTranslations(
      allTranslationTasks,
      fileContent,
      progressCallback,
    );
      ...
      const batchPromises = batch.map(async (task) => {
        try {
          const translatedText = await this.translateText(
            ...
            task.targetLanguage,
            task.key,
            fileContent.strings[task.key].comment,
          );
          ...
          console.error(
            `Translation failed for ${task.text} to ${task.langCode}:`,
            error,
          );
  ...
  async translateXliffFilePaths(filePaths: string[]): Promise<void> {
    ...
    const promises = allXliffFiles.map((filePath) =>
      this.translateXliffFilePath(filePath),
    );
    ...
    for (let i = 0; i < tasks.length; i += this.concurrencyLimit) {
      ...
      const batchPromises = batch.map(async (task) => {
        try {
          ...
          console.error(
            `Translation failed for ${task.sourceText} to ${task.targetLanguage}:`,
            error,
          );
          return { success: false, task, error };
        }
  ...
  private extractContext(key: string, comment?: string): string {
    let context = '';

    // 从key中提取上下文信息
    const keyParts = key.split(/[._-]/);
    if (keyParts.length > 1) {
      context += `Context: ${keyParts.slice(0, -1).join(' -> ')}. `;
    }
  ...
  private async translateText(
    ...
    try {
      ...
      // 检查翻译记忆，确保一致性
      const memoryKey = `${text}_${targetLanguage}`;
      if (this.translationMemory.has(memoryKey)) {
        return this.translationMemory.get(memoryKey)!;
      }

      // 提取上下文信息
      const context = key ? this.extractContext(key, comment) : '';

      // 构建智能prompt
      const messageContent = `You are a professional UI/UX translator specializing in mobile and desktop applications. 

CONTEXT:
- App Type: Desktop/Mobile Application
- String Key: ${key || 'N/A'}
${context ? `- Additional Context: ${context}` : ''}

TRANSLATION GUIDELINES:
1. Analyze the string key and content to understand the UI element type (button, menu, error message, placeholder, etc.)
2. Choose appropriate tone and style for the specific UI element type
3. Maintain consistency with standard UI patterns in ${targetLanguage}
4. Consider character length constraints for UI elements
5. Use natural, user-friendly language that feels native to ${targetLanguage} speakers
6. For buttons: use concise action verbs
7. For error messages: be clear and helpful, not alarming
8. For placeholders: be instructive and brief
9. For technical terms: use commonly accepted translations in the target language
10. Ensure the translation fits the app context and user experience

SOURCE TEXT: "${text}"
TARGET LANGUAGE: ${targetLanguage}

Please provide ONLY the translated text, no explanations or additional formatting:`;
      ...
      // 保存到翻译记忆
      if (result) {
        this.translationMemory.set(memoryKey, result);
      }
